...
export class AddProjectClass extends Component {
  ...
  getSalesStaff = () => {
    ...
    this.props
      .getCommonApi(`employeestafflist/?sales_staff=1&page=1&limit=1000`)
      .then((res) => {
        // activeMenu = []
        console.log("res.data", res);
        let sortedSalesStaffList = res.data.sort(
          this.dynamicSort("display_name")
        );
  ...
  getOperationStaff = () => {
    ...
    this.props
      .getCommonApi(`employeestafflist/?sales_staff=2&page=1&limit=1000`)
      .then((res) => {
        // activeMenu = []
        console.log("res.data", res);
        let sortedOperationalStaffList = res.data.sort(
          this.dynamicSort("display_name")
        );
        for (let key of sortedOperationalStaffList) {
  ...
  getAdminStaff = () => {
    ...
    this.props
      .getCommonApi(`employeestafflist/?sales_staff=0&page=1&limit=1000`)
      .then((res) => {
        // activeMenu = []
        console.log("res.data", res);
        let sortedAdminStaffList = res.data.sort(
          this.dynamicSort("display_name")
        );
        for (let key of sortedAdminStaffList) {
  ...
  handleChange = ({ target: { value, name } }) => {
    ...
    if (name == "salesStaff") {
      let checkSameName =
        ...
        this.state.salesStaffTag.filter((tag) => tag === value);
      ...
      let checkSameName =
        ...
        this.state.operationStaffTag.filter((tag) => tag === value);
      if (checkSameName && checkSameName == value) {
        alert("This Member already exist");
      } else {
      ...
      let checkSameName =
        ...
        this.state.adminStaffTag.filter((tag) => tag === value);
      if (checkSameName && checkSameName == value) {
        alert("This Member already exist");
      } else {
