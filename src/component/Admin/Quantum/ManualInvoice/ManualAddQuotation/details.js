...
export class DetailsClass extends React.Component {
  ...
  getTransactionHistory = () => {
    this.props
      .getCommonApi(
        `transactionhistory/?invoice_type=All&sales_staffs=0&page=1&limit=1000`
      )
      .then((res) => {
        ...
        if (res.status == 200) {
          const arrayTransaction = res.data.dataList.map((data, key) => {
            return {
              label: `${data.sa_transacno_ref} - ${data.sa_custname}`,
              value: data.id,
            };
          });
  ...
  getIndividualTransaction = (value) => {
    this.props.getCommonApi(`transactionhistory/${value}`).then((res) => {
      console.log("res in transaction history", res);
      if (res.status == 200) {
        const arrayItemsTranasaction = res.data.daud_lines.map((data) => {
          return {
